...
export class AiService {
  async classifyUpdate(content: string): Promise<{ type: UpdateType; visibility: VisibilityLevel }> {
  ...
  async summarizePlan(planId: string): Promise<string> {
    return 'This is a mock summary of the plan updates.';
  }

  async detectStall(planId: string): Promise<boolean> {
    return false;
  }
}
